...
if($_SERVER['REQUEST_METHOD'] === 'GET'){
	$GLOBALS['APPLICATION']->SetTitle(GetMessage('STROY_CONTROL_CENTER_TITLE'));
}
...
if($RIGHT >= 'R'){
	if(CModule::IncludeModule($moduleID)){
		if($_SERVER['REQUEST_METHOD'] === 'GET'){
			...
			$.ajax({
				...
				error: function(){
					...
					$('#aspro_admin_area').append('<div class="adm-info-message-wrap adm-info-message-red"><div class="adm-info-message"><div class="adm-info-message-title"><?=GetMessage('STROY_MODULE_CONTROL_CENTER_ERROR')?></div><div class="adm-info-message-icon"></div></div></div>');
				}
			});
			</script>
			<?
		...
		CAdminMessage::ShowMessage(GetMessage('STROY_MODULE_NOT_INCLUDED'));
	}
}
else{
	CAdminMessage::ShowMessage(GetMessage('STROY_NO_RIGHTS_FOR_VIEWING'));
}
?>
<?
if($_SERVER['REQUEST_METHOD'] === 'GET'){
