...
if(!function_exists("replaceEventMark")){
	function replaceEventMark($id){
		$admin_type="ASPRO_SEND_FORM_ADMIN_".$id;
		$rsET = CEventType::GetList(array("TYPE_ID" => $admin_type));
		while ($arET = $rsET->Fetch()){
			if(strpos($arET["DESCRIPTION"], "#PAGE_LINK#")===false)
				CEventType::Update(array("ID" => $arET["ID"]), array("DESCRIPTION"=>$arET["DESCRIPTION"].PHP_EOL.GetMes("Адрес страницы:")." #PAGE_LINK#"));
		}
		$arMess = CEventMessage::GetList($by="site_id", $order="desc", array("TYPE_ID"=>$admin_type))->Fetch();
		if(strpos($arMess["MESSAGE"], "#PAGE_LINK#")===false){
			$em = new CEventMessage;
			$res = $em->Update($arMess["ID"], array("MESSAGE"=>$arMess["MESSAGE"].PHP_EOL.PHP_EOL.GetMes("Адрес страницы:")." #PAGE_LINK#"));
		}
	}
}
?>